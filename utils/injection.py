...
def inject_list(
    ...
    if debug:
        logger.info(f"New Item(s): {str(new_items)}")
        logger.info(
            "Injecting List • Owner: {}, Key: {}\n Original List: {}\n Final List: {}".format(
...
def merge_affordance_filter(
    tunable,
    other_filter=None,
    include_all_by_default=DEFAULT,
    include_affordances=(),
    exclude_affordances=(),
    include_lists=(),
    exclude_lists=(),
    ...
    if debug:
        logger.info(
            "Injecting Filter • Original Default Inclusion: {}".format(
                default_inclusion
            )
        )
    ...
    overrides.include_affordances = merge_list(
        default_inclusion.include_affordances, overrides.include_affordances
    )
    overrides.exclude_affordances = merge_list(
        default_inclusion.exclude_affordances, overrides.exclude_affordances
    )
    overrides.include_lists = merge_list(
        default_inclusion.include_lists, overrides.include_lists
    )
    overrides.exclude_lists = merge_list(
        default_inclusion.exclude_lists, overrides.exclude_lists
    )
    ...
    if debug:
        logger.info(
            "Injecting Filter • New Default Inclusion: {}".format(new_default_inclusion)
        )
...
def inject_to(
    ...
    def _inject(new_function):
        ...
        setattr(
            ...
            _wrap_target(target_function, new_function),
        )
        return new_function
