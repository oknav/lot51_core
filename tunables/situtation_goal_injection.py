...
class TunableSituationGoalInjection(BaseTunableInjection):
    FACTORY_TUNABLES = {
        "situation_goal": TunableReference(
            manager=get_instance_manager(Types.SITUATION_GOAL), pack_safe=True
        ),
        "_goal_loot_list": TunableList(
        ...
        "_pre_tests": TunableSituationGoalPreTestSet(
        ...
        "_target_tests": TunableTargetedSimTestSet(
    ...
    def inject(self):
        ...
        inject_list(
            self.situation_goal, "_goal_loot_list", self._goal_loot_list, debug=True
        )
        inject_list(self.situation_goal, "_pre_tests", self._pre_tests, debug=True)
        inject_list(
            self.situation_goal, "_target_tests", self._target_tests, debug=True
        )
